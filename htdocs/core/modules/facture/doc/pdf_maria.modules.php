 ...
 */
class pdf_maria extends ModelePDFFactures
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		// Grouper les lignes de produits identiques
		$object->lines = $this->groupIdenticalProducts($object);
		$nblines = count($object->lines); // Mettre à jour le nombre de lignes après regroupement
	...
	protected function groupIdenticalProducts($object)
	{
		$groupedLines = array();
		
		foreach ($object->lines as $line) {
			$key = $line->product_ref . '-' . $line->tva_tx;
			
			if (!isset($groupedLines[$key])) {
				$groupedLines[$key] = $line;
			} else {
				$groupedLines[$key]->qty += $line->qty;
				$groupedLines[$key]->total_ht += $line->total_ht;
				$groupedLines[$key]->total_tva += $line->total_tva;
				$groupedLines[$key]->total_ttc += $line->total_ttc;
			}
		}
